...
/**
 * ContactForm — Reusable dark enquiry form
 *
 * Props:
 *  @param {string}   logoSrc        - Path/URL to logo image (optional)
 *  @param {string}   logoAlt        - Alt text for the logo
 *  @param {string}   brandName      - Fallback brand name if no logo (optional)
 *  @param {string}   accentColor    - Primary accent hex color (default: #e36a2e)
 *  @param {string}   apiUrl         - POST endpoint URL
 *  @param {string}   successPath    - Route to navigate after success (default: /success-page)
 *  @param {string[]} radioOptions   - List of service options (uses DEFAULT_RADIO_OPTIONS if omitted)
 *  @param {string}   formTitle      - Form heading (default: "Send us a message")
 *  @param {string}   formSubtitle   - Small label above heading (default: "Let's Talk")
 *  @param {string}   submitLabel    - Submit button text (default: "Submit Request")
 *  @param {string}   footerNote     - Small note below button
 *  @param {object}   extraFields    - Additional fields beyond default 4 (see below)
...
const ContactForm = ({
  ...
  const accentFaint = `${accent}1F`; // ~12% opacity
  const accentMid = `${accent}99`; // ~60% opacity
  ...
  return (
    ...
    >
      ...
      />
      {/* Decorative corner glow bottom-left */}
      <div
      ...
      <form onSubmit={handleSubmit} className="relative p-7 sm:p-9">
        {/* Logo / Brand */}
        <div
          className="flex items-center gap-2 mb-7 pb-6"
          ...
          {inputFields.map(({ ph, name, type }) => (
            <div key={name} className="relative group">
              ...
              <input
                ...
                onBlur={() => setFocused("")}
                className="w-full pl-5 pr-10 py-3.5 rounded-xl text-white text-sm
                  placeholder:text-zinc-600 focus:outline-none transition-all duration-200 font-medium"
                style={{
                  background:
                    focused === name
        ...
        {/* Radio list */}
        <div
          className="space-y-0.5 mb-7 max-h-52 overflow-y-auto pr-1"
          style={{
            scrollbarWidth: "thin",
        ...
        >
          {radioOptions.map((opt) => {
            ...
            return (
              <label
                key={opt}
                className="flex items-center gap-3 px-4 py-2.5 rounded-xl cursor-pointer transition-all duration-150 relative"
                style={{
              ...
              >
                ...
                <input
                  ...
                  value={opt}
                  checked={active}
                  onChange={() => setFormData((p) => ({ ...p, service: opt }))}
                  className="sr-only"
                />
                <span
        ...
        <button
          type="submit"
          className="group relative w-full overflow-hidden font-black text-xs tracking-[0.2em] uppercase
            px-8 py-4 rounded-xl flex items-center justify-center gap-2.5 transition-all duration-300 text-white"
          style={{
            background: `linear-gradient(135deg, ${accent} 0%, ${accent}dd 100%)`,
            boxShadow: `0 4px 16px ${accent}40`,
          }}
          onMouseEnter={(e) => {
            e.currentTarget.style.background = `linear-gradient(135deg, ${accent}ee 0%, ${accent}bb 100%)`;
            e.currentTarget.style.boxShadow = `0 8px 32px ${accent}70`;
            e.currentTarget.style.transform = "translateY(-1px)";
          }}
          onMouseLeave={(e) => {
            e.currentTarget.style.background = `linear-gradient(135deg, ${accent} 0%, ${accent}dd 100%)`;
...
//   <ContactForm logoSrc={logo} logoAlt="My Brand" />
//
// Custom accent color + custom services:
//   <ContactForm
//     logoSrc={logo}
//     accentColor="#3b82f6"
//     radioOptions={["SEO", "PPC", "Web Design"]}
//     formTitle="Book a Free Call"
//     formSubtitle="Get Started"
//     submitLabel="Book Now"
//     footerNote="No credit card required."
//   />
//
// No logo, show brand name text instead:
//   <ContactForm brandName="GrowthFlow Media" />
//
// Different API + success route:
//   <ContactForm
//     logoSrc={logo}
//     apiUrl="https://my-api.com/enquiry"
